...
# Shift IDs que vamos usar (devem existir no mesmo dia)
SHIFT_IDS = [1, 2, 3]

# 🔹 Login usando form-data e obter token
def login_user(user):
    resp = requests.post(f"{BASE_URL}/users/login", data={
        "username": user["email"],  # campo username, valor é o email
...
# 🔹 Verificar que todos os swaps são do mesmo dia
...
# 🔹 Confirmar ciclo por todos os usuários
